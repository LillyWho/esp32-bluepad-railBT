...
bool invertLeftStick = true;
bool invertRightStick = true;
/////  Also, let's define the maximum the sticks report so we can lerp accordingly.
long maxYAxisL = 509;
long maxYAxisR = 509;
long yAxisL = 0;
long yAxisR = 0;
...
void mode1() {
  ...
  float directionLerp = lerp(yAxisL, -509, 509, -4, 4);
  ...
  if (direction == 0) {
    throttleTick = 0;
  } else if (direction != 0 && millis() - throttleTick > 150 && ((directionPlus && targetSpeed < maxSpeed) || (directionMinus && targetSpeed > (maxSpeed * -1)))) {
    throttleTick = millis();
